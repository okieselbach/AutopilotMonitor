...
- Interim runs are bounded by trigger frequency, and the chosen v1 triggers are rare by construction (`hybrid_login_pending` ≤1 per boot, `whiteglove_sealed` once per session). A tenant authoring `on_event:performance_snapshot` would be expensive — the trigger registry intersection happens per ingest batch, so worst case is one envelope per batch; if that ever hurts, a per-(session, reason) throttle marker is the v2 lever. The lint should warn on high-frequency trigger types (catalog already knows them).
