...
try {
    ...
    if ($profilePaths.Count -gt 0 -and -not $relaxActive) {
        ...
        Write-Log "SKIP: Real user profile(s) found ($details). OOBE state: $oobeState. CloudPC: $([bool]$cloudPcModel). Device appears productive."
        exit 0
    }
    ...
    if ($lastLoggedOnUser -and $lastLoggedOnUser -notmatch 'defaultuser\d*' -and -not $relaxActive) {
    ...
    Write-Log "Pre-flight checks passed -- no prior deployment, no productive-device signals, within bootstrap window (relaxActive=$relaxActive)"
