...
namespace AutopilotMonitor.Functions.Services
{
    ...
    public class TenantConfigPatchService
    {
        ...
        internal static readonly HashSet<string> BaseDeniedFields = new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            "ProDowngradedUtc", "MaxDelegatedTenantsOverride",
        };
        ...
        /// <summary>
        /// Machine-readable schema of the tenant-config fields for the MCP write surface:
        /// name (camelCase, as the patch endpoint accepts it), JSON type, nullability
        /// (nullable ⇒ an explicit JSON null clears the field), writability incl. the deny
        /// reason, the phase-2 GA-only marker, and the revert-protected marker. Generated by
        /// REFLECTION over the model + the deny-lists above — the same sources the patch flow
        /// enforces — so the schema can never drift from what the endpoint actually accepts.
        /// Static per process (the model does not change at runtime).
        /// </summary>
        internal static IReadOnlyList<TenantConfigFieldSchema> BuildFieldsSchema() => FieldsSchema.Value;

        private static readonly Lazy<IReadOnlyList<TenantConfigFieldSchema>> FieldsSchema = new(() =>
        {
            var nullability = new NullabilityInfoContext();
            var fields = new List<TenantConfigFieldSchema>();
            foreach (var prop in ModelProperties.Values)
            {
                ...
                var type =
                    ...
                    : underlying == typeof(decimal) || underlying == typeof(double) || underlying == typeof(float) ? "number"
                    : "string";
                var format =
                    underlying == typeof(DateTime) || underlying == typeof(DateTimeOffset) ? "date-time"
                    : prop.Name.EndsWith("Json", StringComparison.Ordinal) ? "json"
                    : null;
        ...
        private static string DeniedReason(string name) => name switch
        {
            ...
            or "MaxDelegatedTenantsOverride"
