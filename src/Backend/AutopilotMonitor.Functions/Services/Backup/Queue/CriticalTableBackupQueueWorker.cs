    ...
    public sealed class CriticalTableBackupQueueWorker : BackgroundService
    {
        ...
        private async Task ProcessJobAsync(string jobId, QueueMessage msg, CancellationToken ct)
        {
            ...
            try
            {
                ...
                var casOk = await _jobs.TryUpdateWithCasAsync(runningJob, jobETag!.Value, ct).ConfigureAwait(false);
                if (!casOk)
                {
                    // Concurrent state change (watchdog, parallel worker). Re-read + dispatch.
                    var (fresh, _) = await _jobs.GetWithETagAsync(jobId, ct).ConfigureAwait(false);
                ...
                try
                {
                    ...
                    var (currentJob, currentETag) = await _jobs.GetWithETagAsync(jobId, ct).ConfigureAwait(false);
                    if (currentJob is not null && currentETag is not null)
                    {
                        ...
                        await _jobs.TryUpdateWithCasAsync(currentJob, currentETag.Value, ct).ConfigureAwait(false);
                    }

                    result = await _backupService.RunBackupUnderLeaseAsync(backupId, runningJob.RequestedBy, ct).ConfigureAwait(false);
                }
                ...
                if (retryableException is not null)
                {
                    ...
                    if (rb is not null && rbETag is not null)
                    {
                        ...
                        rb.LastHeartbeatUtc = _clock.GetUtcNow().UtcDateTime;
                ...
                var (finalJob, finalETag) = await _jobs.GetWithETagAsync(jobId, ct).ConfigureAwait(false);
                if (finalJob is not null && finalETag is not null)
                {
                    ...
                    finalJob.Error = null;
                    await _jobs.TryUpdateWithCasAsync(finalJob, finalETag.Value, ct).ConfigureAwait(false);
                }
            ...
            finally
            {
                ...
                if (lease is not null)
                {
                    ...
                    catch (Exception ex) { _logger.LogWarning(ex, "CriticalTableBackupQueueWorker: lease release failed (auto-expires)"); }
                }
            }
