 ...
 * (ops) channel section in the admin area. One component so the two never drift on provider
...
export const PROVIDERS: {
  ...
  // Telegram sends through the PLATFORM bot, so only a Global Admin may configure one. The
  // dropdown hides it for everyone else; the server enforces the same rule (a hidden option is
...
/** Single channel card: name, provider, destination, headers (generic only), optional event
 *  subscriptions, test. */
export function ChannelEditor({
  ...
  return (
    <div className={`rounded-lg border ${channel.enabled ? "border-gray-200" : "border-gray-100 bg-gray-50"} p-4 space-y-4`}>
      ...
      {/* Event subscriptions — tenant channels only; ops channels are targeted by alert rules. */}
      {showEventToggles && (
        <div>
