 ...
 *
 * Two hard safety rules learned from a production redirect loop (2026-07-29):
 *
 ...
 *
 * 2. Loop breaker: auth state is per-origin. If the two origins disagree about
...
export function HostRoutingGuard() {
  ...
  useEffect(() => {
    ...
    if (isOnPublicHost() && !isPublicPath(pathname)) {
      if (registerBounce()) window.location.replace(`https://${PORTAL_HOST}${suffix}`);
    } else if (isOnPortalHost() && isPublicPath(pathname)) {
      if (registerBounce()) window.location.replace(`https://${PUBLIC_HOST}${suffix}`);
    }
  }, [pathname]);
