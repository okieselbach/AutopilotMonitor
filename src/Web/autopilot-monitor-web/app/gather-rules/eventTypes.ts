...
export const KNOWN_EVENT_TYPES: EventTypeEntry[] = [
  ...
  { value: "script_recurrence_summary", label: "script_recurrence_summary", category: "script",
    description: "Per health-script policy, when the agent stops: further runs that repeated the last reported result were counted instead of emitted. IME re-runs remediation policies on its own schedule for as long as the device is up; the first run and every run with a different result (compliance, exit code, stderr, remediation status) still emit script_started / script_completed / script_failed in full. Carries runsObserved, runsCollapsed, runsWithoutResult, collapsedResults, firstCollapsedAt and lastCollapsedAt." },
  ...
  { value: "system_clock_changed", label: "system_clock_changed", category: "diagnostics",
    description: "The OS clock was stepped (System log, Kernel-General EventID 1) — ground truth for time jumps: oldTime/newTime (ISO-8601, authoritative — the event's own timestamp may be clamped), signed timeDeltaMs, reason/reasonText (application_set / hardware_clock_sync), processName/processId of the setter. Steps below 2 s (w32time micro-slews) are suppressed; steps of 5+ min are Warning with immediate upload. Backfilled from before agent start (backfilled=true) — catches the early-OOBE w32time correction of a wrong BIOS clock." },
  { value: "system_sleep_episode", label: "system_sleep_episode", category: "diagnostics",
    description: "A completed sleep episode, emitted retroactively at wake: kind=sleep/hibernate (Power-Troubleshooter EventID 1, carries wakeSourceType/wakeSourceText) or modern_standby (Kernel-Power EventID 507, carries sleepDurationSeconds, reason, onAcPower, batteryRemainingCapacityOnExit). Payload enteredAt/exitedAt (ISO-8601, authoritative) and durationSeconds explain timeline gaps and inflated wall-clock durations. Modern-standby episodes under 60 s of real sleep are suppressed; pre-agent episodes are backfilled (backfilled=true)." },
